...
Dialog.addString("File path for output:", path);
...
if (check_tif == true) {
	File.makeDirectory(path_output + "_tif");
}
if (check_projection == true) {
	File.makeDirectory(path_output + "_projection");
}
if (check_preview == true) {
	File.makeDirectory(path_output + "_preview");
	File.makeDirectory(path_output + "_preview/temp1");
	File.makeDirectory(path_output + "_preview/temp2");
}
...
for (a = 0; a<lengthOf(list_stacks); a++) {
	...
	if (check_tif == true) {
		saveAs("Tiff", path_output + "_tif/" + list_stacks[a]);
	}
	...
	if (check_projection == true) {
		...
		saveAs("Tiff", path_output + "_projection/" + list_stacks[a]);
		close();
	}
	...
	if (check_preview == true) {
		...
		run("8-bit");
		//save frame
		saveAs("Tiff", path_output + "_preview/temp1/" + list_stacks[a]);
		close;
	}
	close();
}
...
if (check_preview == true){
	...
	for (i=0; i<lengthOf(list_preview); i=i+nr_channels){
		...
		saveAs("Tiff", path_output + "_preview/temp2/" + list_preview[i]);
		close();
		for (k=0; k<nr_channels; k++) {
			//close();
		}
	...
	for (i=0; i<nr_movies+1; i=i+nr_movies) {
		...
		if (check_timestamp == true) {
			...
			run("AVI... ", "compression=JPEG frame=" + preview_framerate + " save=[" + path_output + "_preview/preview_" + count_movies + ".avi" + "]");
			close();
		}
	}
}
...
IJ.log("---");
selectWindow("Log");
saveAs("Text", path_output + "FlamingoConverter_log_" + TimeString + ".txt");
