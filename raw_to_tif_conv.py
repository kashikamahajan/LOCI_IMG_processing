...
def run():
    ...
    if raw_to_8bit:
        for f in file_names:
            raw_to_8bit_tiff(f, px_x, px_y,z_slices)
    if raw_to_16bit:
        for f in file_names:
            raw_to_16bit_tiff(f, px_x, px_y,z_slices)

    sys.exit()


def raw_to_16bit_tiff(path: str, x: int, y: int, z: int):
    """
...
def raw_to_8bit_tiff(path: str, x: int, y: int, z:int):
    """
